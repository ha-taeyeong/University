  ...
  <header class="bg-white shadow">
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      ...
      <div class="hidden lg:flex lg:flex-1 lg:justify-end">
        <router-link to="/login" class="text-sm font-semibold text-gray-900 hover:text-indigo-600">로그인 &rarr;</router-link>
      </div>
    </nav>
    <!-- 모바일 메뉴 (open=true일 때 보이게) -->
    <div v-if="open" class="lg:hidden">
      <div class="px-6 pb-6">
        <router-link to="/list" class="block py-2 text-base font-semibold text-gray-900 hover:text-indigo-600">리스트</router-link>
        <router-link to="/me" class="block py-2 text-base font-semibold text-gray-900 hover:text-indigo-600">프로필</router-link>
        <button @click="logout" class="block py-2 text-base font-semibold text-gray-900 hover:text-indigo-600 w-full text-left">로그아웃</button>
      </div>
    </div>
  </header>
</template>
