...
public class Prac18 {
    public static void main(String[] args) {
        ...
        int candyCnt = sc.nextInt(); // 사탕 갯수
        int count = 0; // 경우의 수

        // t:택희, n:남규, y:영훈
        for (int t = 1; t < candyCnt; t++) {
            if (t % 2 != 0) continue; // 택희가 받는 사탕의 수는 홀수개가 되어서는 안 된다.

            // 택희 주고 남은 사탕 나누기
            for (int y = 1; y < candyCnt - t; y++) {
                int n = candyCnt - t - y; // 남규는 남은 사탕 전부
                if(n <= 0) continue;

                if(n >= y + 2) { // 남규는 영훈이보다 2개 이상 많은 사탕을 가져야 한다.
                    count++;
                }
            }
        }
